...
BEGIN
    ...
    IF @AttributeProductCreatorId IS NULL OR @AttributeProductCreatorId <= 0
    BEGIN
        SET @o_code = -1;
        SET @o_message = 'El ID del creador (@AttributeProductCreatorId) es obligatorio y debe ser mayor a cero.';
        RETURN;
    END;
    ...
    IF NOT EXISTS (SELECT 1 FROM [SQM_SECURITY].[Tbl_Users] WHERE userId = @AttributeProductCreatorId AND userStatusId = 1)
    BEGIN
        SET @o_code = -1;
    ...
    IF EXISTS (SELECT 1 FROM [SQM_CATALOGS].[Tbl_AttributeProducts] WHERE AttributeProductName = TRIM(@AttributeProductName) AND AttributeProductStatusId = 1)
    BEGIN
        SET @o_code = -1;
        SET @o_message = 'Ya existe un atributo activo con este nombre.';
        RETURN;
    END;
    ...
    BEGIN TRY
        ...
        (
            @AttributeProductAttributesTypeId,
            TRIM(@AttributeProductName),
            TRIM(@AttributeProductDescription),
            @AttributeProductCreatorId,
            GETDATE(),
            @AttributeProductStatusId
        );
        ...
        SET @o_code = 200;
        SET @o_message = 'Atributo creado correctamente.';
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
