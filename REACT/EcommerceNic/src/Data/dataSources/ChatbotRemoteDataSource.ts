...
class ChatbotRemoteDataSource {
  ...
  async getConversations(userId?: string): Promise<ChatbotConversationResponse[]> {
    try {
      const url = userId ? `${API_URL}/api/chatbot/conversations?userId=${encodeURIComponent(userId)}` : `${API_URL}/api/chatbot/conversations`;
  ...
  async getConversation(conversationId: string): Promise<ChatbotConversationResponse | null> {
    try {
      const response = await fetch(`${API_URL}/api/chatbot/conversations/${encodeURIComponent(conversationId)}`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' },
      });
      if (!response.ok) return null;
